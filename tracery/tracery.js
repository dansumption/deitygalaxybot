...
const setRng = function setRng(newrandomNumberGenerator) {
    randomNumberGenerator = newrandomNumberGenerator;
};

const TraceryNode = function(parent, childIndex, settings) {
    this.errors = [];
    ...
    switch (this.type) {
        case 0:
            ...
            for (let i = 0; i < this.ruleSections.length; i++) {
                const n = new TraceryNode(grammar, 0, {
                    type: -1,
...
RuleSet.prototype.selectRule = function(errors) {
    ...
    if (this.defaultRules !== undefined) {
        ...
        switch (distribution) {
            ...
            default:
                index = Math.floor(
                    Math.pow(randomNumberGenerator(), this.falloff) *
                        this.defaultRules.length
                );
                break;
        }
...
function fyshuffle(array) {
    ...
    while (0 !== currentIndex) {
        ...
        randomIndex = Math.floor(randomNumberGenerator() * currentIndex);
        currentIndex -= 1;
